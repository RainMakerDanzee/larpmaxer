...
/**
 * True when a message came from a content script in a web page, rather than
 * from one of the extension's own surfaces.
 */
function isFromPage(sender: chrome.runtime.MessageSender): boolean {
...
async function route(msg: Message, sender: chrome.runtime.MessageSender): Promise<void> {
  ...
  // were sent, so for messages arriving from a page the sender is
  // authoritative. Extension surfaces are excluded even when they happen to be
  // in a tab: they address a tab other than their own, and overwriting that
  // with their own id would point the run at the extension's own page. The
  // side panel has no sender.tab so it was never affected, but an options page
  // or popup would have been.
  if (isFromPage(sender) && "tabId" in msg) {
