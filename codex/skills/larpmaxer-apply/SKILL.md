...
ledger). Follow the workspace `AGENTS.md`'s "Applying (the agent loop)" section exactly — the
...
- **Canonicalize pasted links.** `linkedin.com/jobs/search*?currentJobId=N` is
  ...
  search page.
- **Open the browser** and read the full ad before writing a word.
