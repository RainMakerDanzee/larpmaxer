# LarpMaxer Agent — Operating Manual
...
2. **Approval gates.** Research, drafting, and form-filling are autonomous.
   ...
   them log in in the browser pane — never handle credentials yourself.
...
Created by `/larpmaxer:intake` in the folder where the user runs Claude:
...
1. **Canonicalize.** A pasted LinkedIn `/jobs/search*?currentJobId=N` URL means
   ...
   the browser pane and read the whole ad.
